...
public class CListener implements MessageListener{  
    ...
    private void operate(String s) {
    	...
    	if (begin!=0) {
        	...
        	CSendback server = new CSendback();
        	String password = "1129009";
        	crawler.crawler(begin, password, server);
		}
    	else {
			CSendback sendback = new CSendback();
			sendback.sendback();
		}
    ...
    public static void main(String[] args) throws Exception {  
        ...
        conn.start();  
        //消息目的地  
        Destination dest = session.createQueue("c");
        //消息消费者  
        MessageConsumer consumer = session.createConsumer(dest);  
        consumer.setMessageListener(new CListener());  
